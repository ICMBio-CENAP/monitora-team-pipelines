...
rn_data_juruena <- rn_data_juruena %>%
  distinct(placename, sampling_event, species, .keep_all = TRUE) %>%
  print()

site <- as.numeric(dense_rank(rn_data_juruena$placename))
species <- as.numeric(dense_rank(rn_data_juruena$species))
year <- match(rn_data_juruena$sampling_event, 2016:2024)
trials <- as.numeric(rn_data_juruena$trials)
y <- as.numeric(rn_data_juruena$y)

#rm(rn_data_2002545)

min(rn_data_juruena$sampling_event)
max(rn_data_juruena$sampling_event)
...
# save results
saveRDS(out, here("output", "model_juruena.rds"))
...
N <- tibble(especie = unique(rn_data_juruena$species)) %>%
...
Nhat <- tibble(especie = unique(rn_data_juruena$species)) %>%
...
tibble(ano = sort(unique(rn_data_juruena$sampling_event)),
       sampled = "yes")
