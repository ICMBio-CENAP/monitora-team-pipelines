...
rn_data_jamari[1746,] # y > trials...


site <- as.numeric(dense_rank(rn_data_jamari$placename))
species <- as.numeric(dense_rank(rn_data_jamari$species))
year <- match(rn_data_jamari$sampling_event, 2016:2024)
trials <- as.numeric(rn_data_jamari$trials)
y <- as.numeric(rn_data_jamari$y)

#rm(rn_data_2002545)

min(rn_data_jamari$sampling_event)
max(rn_data_jamari$sampling_event)
...
# save results
saveRDS(out, here("output", "model_jamari.rds"))
...
site <- as.numeric(dense_rank(rn_data_jamari$placename))
species <- as.numeric(dense_rank(rn_data_jamari$species))
year <- as.numeric(dense_rank(rn_data_jamari$sampling_event))
trials <- as.numeric(rn_data_jamari$trials)
y <- as.numeric(rn_data_jamari$y)
...
tibble(ano = sort(unique(rn_data_jamari$sampling_event)),
       sampled = "yes")
